...
public class UserController {
    ...
    public Object login(@RequestParam(value = "username") String username, @RequestParam(value = "password") String password) {
        ...
        if (users.size() == 0) {
            return new ResultResponse(ResultResponse.STATUS_FAILED, "用户名错误");
        } else if (users.size() > 1) {
            return new ResultResponse(ResultResponse.STATUS_FAILED, "账号出错");
        }
        User user = users.get(0);
        if (user.checkPassword(password)) {
            user.setToken(generateToken(user.getUsername()));
            mUserDao.updateToken(user);
            return new VUser(user);
        } else {
            return new ResultResponse(ResultResponse.STATUS_FAILED, "密码错误");
        }
    }
